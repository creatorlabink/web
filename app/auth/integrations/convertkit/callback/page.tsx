...
function CallbackContent() {
  ...
  const [message, setMessage] = useState('Finalizing ConvertKit connection...');
  ...
  useEffect(() => {
    ...
    async function finish() {
      if (error) {
        if (!active) return;
        setStatus('error');
        setMessage('ConvertKit denied the connection request.');
        return;
      }
      if (!code || !state) {
        if (!active) return;
        setStatus('error');
        setMessage('Missing OAuth callback parameters from ConvertKit.');
        return;
      }
      try {
        await integrationsApi.convertkitExchangeCode(code, state);
        if (!active) return;
        setStatus('success');
        setMessage('ConvertKit connected. Redirecting to Connected Apps...');
        setTimeout(() => router.replace('/dashboard/apps'), 900);
      } catch {
        if (!active) return;
        setStatus('error');
        setMessage('Failed to connect ConvertKit. Please try again.');
      }
    }
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-8 text-center">
      ...
      <h1 className="text-xl font-bold mb-2">ConvertKit Integration</h1>
...
function Fallback() {
  return (
    <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-8 text-center">
      ...
      <h1 className="text-xl font-bold mb-2">ConvertKit Integration</h1>
      <p className="text-sm text-gray-300">Finalizing ConvertKit connection...</p>
    </div>
  );
}

export default function ConvertKitCallbackPage() {
  return (
